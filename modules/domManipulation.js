import { cartItems } from "./cartService";
...
export function createProductElement(product) {
  ...
  btnDiv.appendChild(cartIconDiv);
...
function createCartBasket(cartItem) {
  const cartDivContainer = document.createElement('div');
  cartDivContainer.classList.add('item-wrapper');

  const cartDiv = document.createElement('div');
  cartDiv.classList.add('py-3', 'd-flex', 'justify-content-between', 'border-bottom', '--bs-light-bg-subtle');
  ...
  itemWrapperDiv.appendChild(cartItemName);

  const itemPriceDiv = document.createElement('div');
  itemPriceDiv.classList.add('d-flex', 'cart-price-wrapper');

  const itemPriceElement1 = document.createElement('p');
  itemPriceElement1.classList.add('color-orange');
  itemPriceElement1.textContent = `${cartItem.quantity}x`;

  const itemPriceElement2 = document.createElement('p');
  itemPriceElement2.classList.add('color-3');
  itemPriceElement2.textContent = `@ $${cartItem.price}`;

  const itemPriceElement3 = document.createElement('p');
  itemPriceElement3.classList.add('color-5');
  itemPriceElement3.textContent = `$${(cartItem.quantity * cartItem.price).toFixed(2)}`;

  itemPriceDiv.appendChild(itemPriceElement1);
  itemPriceDiv.appendChild(itemPriceElement2);
  itemPriceDiv.appendChild(itemPriceElement3);
  itemWrapperDiv.appendChild(itemPriceDiv);

  const cancelBtnDiv = document.createElement('div');
  cancelBtnDiv.classList.add('pt-3');
  const cancelSVG = `<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="none" viewBox="0 0 10 10">
  ...
  cartDiv.appendChild(itemWrapperDiv);
  cartDiv.appendChild(cancelBtnDiv);

  cartDivContainer.appendChild(cartDiv);

  return cartDivContainer;
}
...
export function createModal() {
    ...
    if (typeof bootstrap === 'undefined') {
      ...
      <div class="modal-dialog">
        ...
        <div class="title-container">
          <h2 class="modal-title" id="staticBackdropLabel">Order <br> Confirmed</h2>
        </div>
      ...
      <div id="modalCartItems">
        ${cartItems.length > 0 ? cartItems.map(cartItem => `
          <div class="py-3 d-flex justify-content-between border-bottom --bs-light-bg-subtle">
            <div>
              <h3>${cartItem.name}</h3>
              <div class="d-flex cart-price-wrapper">
                <p class="color-orange">${cartItem.quantity}x</p>
                <p class="color-3">@ $${cartItem.price}</p>
              </div>
            </div>
            <div class="pt-3">
              <p class="color-5">$${(cartItem.quantity * cartItem.price).toFixed(2)}</p>
            </div>
          </div>`).join('') : '<p>Your cart is empty.</p>'}
      </div>
    </div>
          <div class="modal-footer-wrapper">
             <button type="button" class="new-order-btn btn-primary">Start New Order</button>
          </div>
        </div>
      </div>
    </div>
  ...
  if (modalElement) {
    ...
    // Confirm the button click is detected
  });
//   location.reload();
...
export function populateCartItems() {
    ...
    if (!Array.isArray(cartItems)) {
      cartItems = []; // Default to an empty array if cartItems is not an array
    }
  
    // const modal = createModal(cartItems);
    ...
    cartHeading.classList.add('color-orange', 'headingStyle');
    cartHeading.innerHTML = `Your Cart(${totalQuantity})`;
  
    cartItemsContainer.appendChild(cartHeading);
  
    if (totalQuantity > 0) {
      cartItems.forEach((item) => {
        const itemElement = createCartBasket(item);
        cartItemsContainer.appendChild(itemElement);
      });
  
      const orderTotalDiv = document.createElement('div');
      orderTotalDiv.classList.add('order-total-wrapper', 'py-3', 'd-flex', 'justify-content-between');
  
      const orderTotalText = document.createElement('p');
      orderTotalText.classList.add('color-6', 'total-text-wrapper');
      orderTotalText.textContent = 'Order Total';
  
      const cartTotal = document.createElement('h2');
      cartTotal.classList.add('color-6');
      cartTotal.textContent = `$${totalAmount.toFixed(2)}`;
  
      orderTotalDiv.appendChild(orderTotalText);
      orderTotalDiv.appendChild(cartTotal);
  
      const carbonTextDiv = document.createElement('div');
      carbonTextDiv.classList.add('carbon-text-container', 'py-3', 'd-flex');
  
      const treeSVGDiv = document.createElement('div');
      const treeSVG = `<svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" fill="none" viewBox="0 0 21 20"><path fill="#1EA575" d="M8 18.75H6.125V17.5H8V9.729L5.803 8.41l.644-1.072 2.196 1.318a1.256 1.256 0 0 1 .607 1.072V17.5A1.25 1.25 0 0 1 8 18.75Z"/><path fill="#1EA575" d="M14.25 18.75h-1.875a1.25 1.25 0 0 1-1.25-1.25v-6.875h3.75a2.498 2.498 0 0 0 2.488-2.747 2.594 2.594 0 0 0-2.622-2.253h-.99l-.11-.487C13.283 3.56 11.769 2.5 9.875 2.5a3.762 3.762 0 0 0-3.4 2.179l-.194.417-.54-.072A1.876 1.876 0 0 0 5.5 5a2.5 2.5 0 1 0 0 5v1.25a3.75 3.75 0 0 1 0-7.5h.05a5.019 5.019 0 0 1 4.325-2.5c2.3 0 4.182 1.236 4.845 3.125h.02a3.852 3.852 0 0 1 3.868 3.384 3.75 3.75 0 0 1-3.733 4.116h-2.5V17.5h1.875v1.25Z"/></svg>`;
      treeSVGDiv.innerHTML = treeSVG;
  
      const carbonText = document.createElement('p');
      carbonText.classList.add('carbon-text');
      carbonText.innerHTML = 'This is a <span class="carbon-text-span">carbon-neutral</span> delivery';
  
      carbonTextDiv.appendChild(treeSVGDiv);
      carbonTextDiv.appendChild(carbonText);
  
      cartItemsContainer.appendChild(orderTotalDiv);
      cartItemsContainer.appendChild(carbonTextDiv);
  
      const confirmBtn = document.createElement('button');
      confirmBtn.type = 'button';
      confirmBtn.classList.add('confirm-order', 'btn-wrapper', 'confirm-btn');
      ...
      confirmBtn.addEventListener('click', () => {
        ...
        const updatedModal = createModal(updatedCartItems);
